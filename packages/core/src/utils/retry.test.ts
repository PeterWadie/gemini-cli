...
describe('retryWithBackoff', () => {
  ...
  it('should default to 4 maxAttempts if no options are provided', async () => {
    // This function will fail more than 4 times to ensure all retries are used.
    const mockFn = createFailingFunction(15);

    const promise = retryWithBackoff(mockFn);

    await Promise.all([
      expect(promise).rejects.toThrow('Simulated error attempt 4'),
      vi.runAllTimersAsync(),
    ]);
  ...
  it('should default to 4 maxAttempts if options.maxAttempts is undefined', async () => {
    // This function will fail more than 4 times to ensure all retries are used.
    const mockFn = createFailingFunction(15);

    const promise = retryWithBackoff(mockFn, { maxAttempts: undefined });

    // Expect it to fail with the error from the 4th attempt.
    await Promise.all([
      expect(promise).rejects.toThrow('Simulated error attempt 4'),
      vi.runAllTimersAsync(),
    ]);
